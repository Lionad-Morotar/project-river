...
describe('getContributorHsl', () => {
  ...
  it('fixes lightness at 55% in dark mode (default)', () => {
    const meta: ContributorMeta = { name: 'alice', firstCommitDate: '2024-01-15', totalCommits: 10 }
    const color = getContributorHsl(meta, bounds, maxCommits)
    expect(color).toMatch(/55%/)
  })

  it('uses 42% lightness in light mode', () => {
    const meta: ContributorMeta = { name: 'alice', firstCommitDate: '2024-01-15', totalCommits: 10 }
    const color = getContributorHsl(meta, bounds, maxCommits, false)
    expect(color).toMatch(/42%/)
  })
})
...
describe('useContributorColors', () => {
  ...
  it('maps OTHERS_LABEL to #94a3b8 in dark mode (default)', () => {
    const contributors: ContributorMeta[] = [
  ...
  it('maps OTHERS_LABEL to #64748b in light mode', () => {
    const contributors: ContributorMeta[] = [
      { name: OTHERS_LABEL, firstCommitDate: '2024-01-01', totalCommits: 5 },
    ]
    const map = useContributorColors(contributors, false)
    expect(map.get(OTHERS_LABEL)).toBe('#64748b')
  })

// detectProjectEvents — 纯函数库
// 从 projectEvents.worker.ts 抽取，供 server-side tools 和 client-side worker 共享复用
...
export type Severity = 'info' | 'positive' | 'warning'
...
export const defaultConfig: EventDetectionConfig = {
...
export function buildDayStats(dailyData: DailyRow[]): DayStat[] {
...
export function detectContributorEvents(
  dailyData: DailyRow[],
...
export function detectActivityMutations(
  dayStats: DayStat[],
  config: EventDetectionConfig,
): ProjectEvent[] {
  const events: ProjectEvent[] = []
...
export function detectProjectArchived(
  _dailyData: DailyRow[],
  config: EventDetectionConfig,
  earliestDate: string,
  latestDate: string,
